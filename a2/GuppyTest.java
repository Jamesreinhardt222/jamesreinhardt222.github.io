...
//        assertTrue(fry.getGenus().equals("Poecilia"));
    ...
    @Test
    public void spawnDoesNotWorkWithMaleOfAge() {
        Guppy maleGuppy = new Guppy(  "Poecilia",
                "elegans",
                25,
                false,
                3,
                0.75);
        ArrayList<Guppy> result = maleGuppy.spawn();
        assertNull(result);
    }

    @Test
    public void spawnDoesNotWorkWithFemaleUnderAge() {
        Guppy femaleGuppy = new Guppy(  "Poecilia",
                "elegans",
                1,
                true,
                3,
                0.75);
        ArrayList<Guppy> result = femaleGuppy.spawn();
        assertNull(result);
    }

    @Test
    public void spawnDoesWorkWithFemaleOfAge() {
        Guppy femaleGuppy = new Guppy(  "Poecilia",
                "elegans",
                20,
                true,
    ...
    @Test
    public void spawnWorksAroundFiftyPercentOfTheTimeWithFemaleOfAge() {
        Guppy femaleGuppy = new Guppy(  "Poecilia",
                "elegans",
                20,
                true,
                3,
                0.75);
        int amountOfSuccessfulSpawns = 0;
        for (int i = 0; i < 100; i++) {
            ArrayList<Guppy> batch = femaleGuppy.spawn();
            if (batch.size() != 0) {
                amountOfSuccessfulSpawns++;
            }
        }
        assertTrue(40< amountOfSuccessfulSpawns && amountOfSuccessfulSpawns < 60);
    }
